...
export default function Comments(props) {
  ...
  const scrollHandler = (e) => {
    ...
    if (
      document.documentElement.clientHeight + 100 > windowRelativeBottom &&
  ...
  const commentSendHandler = () => {
    if (commentMessage.length < 2) {
      ...
      AddComment(props.userId, props.id, commentMessage)
        .then((data) => {
  ...
  return (
    <div
      className="text-center column justiy-content-center"
      style={{ overflowWrap: "break-word" }}
    >
      <p className="h3">Comments</p>
      <p className="text-danger h5">{errorComment}</p>
      <div className="d-flex m-auto col-md-5 col-sm-5">
        ...
        />
        <Button onClick={commentSendHandler} className="btn btn-primary">
          Send
        </Button>
      </div>
      <div>
